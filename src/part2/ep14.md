# 第 14 期：放到线上给同事用——容器、密钥、成本

第 13 期结束时，这个客服 agent 已经是一个能跑在两个进程上、状态落在
Postgres 里的 HTTP 服务。但它还在你的笔记本上。同事要用，得有一个
...
同事用要么付费要么自己接一个别处的 Postgres。这一期的服务和数据库
...
`uvx` 重装 MCP 服务器、连 Postgres 三段。对同事的体感是：一上午没人用，
第一个人发的第一句话要等一分半。
