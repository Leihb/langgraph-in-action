"""跑第 9 期：
    uv run python -m ep09.main wang t1 "订单 KL-901 想改期，但已经超过免费改期窗口了，我这边突然要住院"
    uv run python -m ep09.main wang t1 "算了，那我态度不好，你们这平台就是坑人"
    uv run python -m ep09.main chen c1 "我们公司想订 15 张东京迪士尼的票"
    uv run python -m ep09.main --resume <user_id> <thread_id> approve
    uv run python -m ep09.main --history <thread_id>
    uv run python -m ep09.main --memory <user_id>

每条 [agent] 回答后面 (prompt_tokens=NNN) 是这一轮真实发给模型的 token 数，
用来对照"清单便宜、正文贵"这句话到底贵多少。
"""
...
from ep09.graph import build_graph
from ep09.mcp_client import load_mcp_tools
from ep09.tools import TOOLS
            ...
            for part in parts:
                for msg in part.get("messages", []):
                    if node == "agent" and msg.tool_calls:
                        ...
                        print(f"[tools] {msg.name} 返回：{str(msg.content)[:100]}")
